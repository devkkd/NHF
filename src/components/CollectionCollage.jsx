...
function Ring({ ring, rotationValue }) {
  const fallbackRotation = useMotionValue(0);
  const usedRotation = rotationValue || fallbackRotation;
  const cardRotation = useTransform(usedRotation, (v) => -v);
    ...
    >
      ...
      {ring.products.map((product, i) => {
        ...
        return (
          ...
          >
            <motion.div style={{ rotate: cardRotation }}>
      ...
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden">
        ...
        >
          ...
          <div
            ...
            style={{
              width: scale < 0.5 ? 24 : 48,
              height: "1.5px",
              backgroundColor: "#7B7F5C",
            }}
          />
          <div className="space-y-1">
            {COLLECTION_TEXT.description.map((line, i) => (
              <p
                key={i}
                className={`${descSize} text-stone-500 font-normal leading-relaxed tracking-wide`}
              >
                {line}
              </p>
            ))}
